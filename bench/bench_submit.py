...
def run(args: List[str], cwd: str, **kwargs) -> subprocess.CompletedProcess:
    return subprocess.run(
        args, cwd=cwd, capture_output=True, text=True, **kwargs
    )
...
def run_benchmark(
    ...
    try:
        # Clone
        git(workdir, "clone", f"https://x-access-token:{token}@github.com/{repo}.git", "repo")
        ...
        for i in range(stack_size):
            ...
            git(repo_dir, "commit", "-m", f"Bench commit {i}\n\nThis is bench commit {i}")
        ...
        if r.returncode != 0:
            print(f"ghstack submit (create) failed:\n{r.stdout}\n{r.stderr}", file=sys.stderr)
        ...
        if r.returncode != 0:
            print(f"ghstack submit (update) failed:\n{r.stdout}\n{r.stderr}", file=sys.stderr)
            raise RuntimeError("ghstack submit failed")
...
def main() -> None:
    ...
    parser.add_argument("--stack-size", type=int, default=3, help="Number of commits in stack")
    parser.add_argument("--username", default=None, help="GitHub username (auto-detected if not set)")
    ...
    for i in range(args.iterations):
        ...
        print(f"create={create.get('total', 0):.0f}ms  update={update.get('total', 0):.0f}ms")
