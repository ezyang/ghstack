...
def main() -> None:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Log debug information to stderr"
    )
    ...
    for subparser in (submit, parser):
        subparser.add_argument(
            "--message", "-m", default="Update", help="Description of change you made"
        )
        subparser.add_argument(
            "--update-fields",
            "-u",
    ...
    land.add_argument(
        "pull_request", metavar="PR", help="GitHub pull request URL of stack to land"
    )
    land.add_argument(
        "--force", action="store_true", help="force land even if the PR is closed"
    )

    checkout = subparsers.add_parser("checkout")
    checkout.add_argument(
        "pull_request", metavar="PR", help="GitHub pull request URL to checkout"
    )
    ...
    action.add_argument(
        "--close", action="store_true", help="Close the specified pull request"
    )
    ...
    with ghstack.logs.manager(debug=args.debug):
        ...
        if args.cmd == "rage":
            ...
            loop.run_until_complete(
                ghstack.status.main(
                    pull_request=args.pull_request, github=github, circleci=circleci
                )
            )
            loop.close()
